...
int64_t VersionSet::maxNextLevelOverlappingBytes() {
	...
	for (int level = 1; level < kNumLevels - 1; level++) {
		for (size_t i = 0; i < version->files[level].size(); i++) {
			const auto f = version->files[level][i];
			version->getOverlappingInputs(level + 1, &f->smallest, &f->largest,
										 &overlaps);
			const int64_t sum = totalFileSize(overlaps);
			if (sum > result) {
				result = sum;
...
Status VersionSet::recover(bool *manifest) {
    // Read "CURRENT" file, which contains a pointer to the version manifest file
    std::string cur;
    Status s = readFileToString(env, currentFileName(dbname), &cur);
    if (!s.ok()) {
        return s;
    }
    ...
    s = env->newSequentialFile(dscname, file);
    if (!s.ok()) {
        if (s.isNotFound()) {
...
void VersionSet::addLiveFiles(std::set <uint64_t> *live) {
	for (int level = 0; level < kNumLevels; level++) {
		auto &files = version->files[level];
		for (size_t i = 0; i < files.size(); i++) {
...
void VersionSet::finalize(Version *v) {
    ...
    for (int level = 0; level < kNumLevels - 1; level++) {
        ...
        if (level == 0) {
            ...
            // Compute the ratio of version size to size limit.
...
bool VersionSet::reuseManifest(const std::string &dscname, const std::string &dscbase) {
    ...
    Status r = env->newAppendableFile(dscname, descriptorFile);
    if (!r.ok()) {
...
int64_t VersionSet::numLevelBytes(int level) const {
    assert(level >= 0);
    assert(level < kNumLevels);
    return totalFileSize(version->files[level]);
}

int VersionSet::numLevelFiles(int level) const {
    assert(level >= 0);
    assert(level < kNumLevels);
    return version->files[level].size();
}
...
Status VersionSet::logAndApply(VersionEdit *edit) {
    ...
    // a temporary file that contains a snapshot of the version version.
    ...
    if (descriptorLog == nullptr) {
        ...
        edit->setNextFile(nextFileNumber);
        s = env->newWritableFile(newManifestFile, descriptorFile);
        if (s.ok()) {
    ...
    if (s.ok() && !newManifestFile.empty()) {
        s = setCurrentFile(env, dbname, manifestFileNumber);
    }
...
const char *VersionSet::levelSummary(LevelSummaryStorage *scratch) const {
    ...
    snprintf(scratch->buffer, sizeof(scratch->buffer),
             "files[ %d %d %d %d %d %d %d ]",
             int(version->files[0].size()),
             int(version->files[1].size()),
             int(version->files[2].size()),
             int(version->files[3].size()),
             int(version->files[4].size()),
             int(version->files[5].size()),
             int(version->files[6].size()));
...
std::shared_ptr <Compaction> VersionSet::pickCompaction() {
    ...
    const bool sizeCompaction = (version->compactionScore >= 1);
    const bool seekCompaction = (version->fileToCompact != nullptr);
    if (sizeCompaction) {
        level = version->compactionLevel;
        assert(level >= 0);
        ...
        if (c->inputs[0].empty()) {
            ...
            c->inputs[0].push_back(version->files[level][0]);
        }
    } else if (seekCompaction) {
        level = version->fileToCompactLevel;
        c.reset(new Compaction(&options, level));
        c->inputs[0].push_back(version->fileToCompact);
    } else {
        return nullptr;
    }
    ...
    if (level == 0) {
        ...
        version->getOverlappingInputs(0, &smallest, &largest, &c->inputs[0]);
...
void VersionSet::setupOtherInputs(const std::shared_ptr <Compaction> &c) {
    ...
    version->getOverlappingInputs(level + 1, &smallest, &largest, &c->inputs[1]);
    ...
    if (!c->inputs[1].empty()) {
        ...
        version->getOverlappingInputs(level, &allStart, &allLimit, &expanded0);
        ...
        if (expanded0.size() > c->inputs[0].size() &&
            ...
            version->getOverlappingInputs(level + 1, &newStart, &newLimit,
                                            &expanded1);
    ...
    if (level + 2 < kNumLevels) {
        version->getOverlappingInputs(level + 2, &allStart, &allLimit,
                                        &c->grandparents);
    }
...
bool Compaction::shouldStopBefore(const std::string_view &internalKey) {
    ...
    if (overlappedBytes > maxGrandParentOverlapBytes(&vset->options)) {
        // Too much overlap for version output; start new output
        overlappedBytes = 0;
        return true;
    } else {
        return false;
