...
Status DBImpl::open() {
    ...
    if (s.ok() && mem == nullptr) {
        ...
        s = env->newWritableFile(logFileName(dbname, newLogNumber), logfile);
        if (s.ok()) {
...
Status DBImpl::recover(VersionEdit *edit, bool *saveManifest) {
	...
	s = env->lockFile(lockFileName(dbname), dbLock);
	if (!s.ok()) {
		return s;
	}
...
Status DBImpl::recoverLogFile(uint64_t logNumber, bool lastLog,
    ...
    if (status.ok() && lastLog && compactions == 0) {
        ...
        if (env->getFileSize(fname, &lfileSize).ok() &&
            env->newAppendableFile(fname, logfile).ok()) {
...
Status DBImpl::makeRoomForWrite(std::unique_lock <std::mutex> &lk, bool force) {
    ...
    while (true) {
		if (!bgerror.ok()) {
			...
			s = env->newWritableFile(logFileName(dbname, newLogNumber), lfile);
			if (!s.ok()) {
...
Status DBImpl::writeLevel0Table(const std::shared_ptr <MemTable> &mem, VersionEdit *edit, Version *base) {
    const uint64_t startMicros = env->nowMicros();
    FileMetaData meta;
    ...
    CompactionStats sta;
    sta.micros = env->nowMicros() - startMicros;
    sta.bytesWritten = meta.fileSize;
    stats[level].add(sta);
    return s;
}

void DBImpl::backgroundCallback() {
	std::unique_lock <std::mutex> lck(mutex);
...
void DBImpl::maybeScheduleCompaction() {
	if (backgroundCompactionScheduled) {
		...
		env->schedule(std::bind(&DBImpl::backgroundCallback, this));
	}
}
...
void DBImpl::testCompactRange(int level, const std::string_view *begin, const std::string_view *end) {
	assert(level >= 0);
...
Status DBImpl::doCompactionWork(CompactionState *compact) {
    ...
    for (; input->valid() && !shuttingdown.load(std::memory_order_acquire);) {
		...
		if (hasimm.load(std::memory_order_relaxed)) {
			const uint64_t immStart = env->nowMicros();
			mutex.lock();
			if (imm != nullptr) {
				compactMemTable();
    ...
    stats.micros = env->nowMicros() - startMicros - immMicros;
...
int64_t DBImpl::testMaxNextLevelOverlappingBytes() {
	std::unique_lock <std::mutex> lk(mutex);
...
Status DBImpl::openCompactionOutputFile(CompactionState *compact) {
    ...
    Status s = env->newWritableFile(fname, compact->outfile);
    if (s.ok()) {
