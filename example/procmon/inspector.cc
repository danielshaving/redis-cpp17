	...
	systemInspector(new SystemInspector)
{
	...
	server.setMessageCallback(std::bind(&Inspector::onRequest, this, std::placeholders::_1, std::placeholders::_2));
	processInspector->registerCommands(this);
	systemInspector->registerCommands(this);
#ifdef HAVE_TCMALLOC
	performanceInspector.reset(new PerformanceInspector);
	performanceInspector->registerCommands(this);
#endif
	loop->runAfter(0, false,std::bind(&Inspector::start, this)); // little race condition
}
...
void Inspector::add(const std::string& module,
	const std::string& command,
	const Callback& cb,
	const std::string& help)
{
	std::unique_lock<std::mutex> lk(mutex);
	modules[module][command] = cb;
	helps[module][command] = help;
}
...
void Inspector::onRequest(const HttpRequest &req, HttpResponse *resp)
{
	if (req.getPath() == "/")
	{
		std::string result;
		std::unique_lock<std::mutex> lk(mutex);
		for (auto helpListI = helps.begin();
			helpListI != helps.end();
			++helpListI)
		{
	...
	else
	{
		...
		else if (result.size() == 1)
		{
			...
			{
				LOG_WARN << "Unimplemented " << module;
			}
		}
		else
		{
			std::string module = result[0];
			auto commListI = modules.find(module);
			if (commListI != modules.end())
			{
				...
				if (it != commList.end())
				{
					...
					if (it->second)
					{
						...
						resp->setBody(cb(req.getMethod(), args));
						ok = true;
					}
				}
			}
