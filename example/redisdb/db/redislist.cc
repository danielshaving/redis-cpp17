...
Status RedisList::LPush(const std::string_view& key,
	...
	*ret = 0;
	...
	if (s.ok()) {
		ParsedListsValue plistsvalue(&metavalue);
		if (plistsvalue.IsStale()
			|| plistsvalue.GetCount() == 0) {
			version = plistsvalue.InitialMetaValue();
		}
		else {
			version = plistsvalue.GetVersion();
		}

		for (const auto& value : values) {
			index = plistsvalue.GetLeftIndex();
			plistsvalue.ModifyLeftIndex(1);
			plistsvalue.ModifyCount(1);
			ListsDataKey listsdatakey(key, version, index);
			batch.Put(listsdatakey.Encode(), value);
		}

		batch.Put(key, metavalue);
		*ret = plistsvalue.GetCount();
	}
	else if (s.IsNotFound()) {
		char str[8];
		EncodeFixed64(str, values.size());
		ListsValue listsvalue(std::string_view(str, sizeof(uint64_t)));
		...
		batch.Put(key, listsvalue.Encode());
...
Status RedisList::LPop(const std::string_view& key, std::string* element) {
	...
	if (s.ok()) {
		...
		else if (plistsvalue.GetCount() == 0) {
			return Status::NotFound("");
		}
		else {
			int32_t version = plistsvalue.GetVersion();
			uint64_t firstnodeindex = plistsvalue.GetLeftIndex() + 1;
...
Status RedisList::LRange(const std::string_view& key, int64_t start, int64_t stop,
	...
	if (s.ok()) {
		...
		if (plistsvalue.IsStale()) {
			...
			int32_t version = plistsvalue.GetVersion();
			uint64_t originleftindex = plistsvalue.GetLeftIndex() + 1;
			uint64_t originrightindex = plistsvalue.GetRightIndex() - 1;
	...
	*ret = 0;
	...
	if (s.ok()) {
		...
		if (plistsvalue.IsStale()) {
			...
			uint32_t version = plistsvalue.GetVersion();
			uint64_t currentindex = plistsvalue.GetLeftIndex() + 1;	
			auto iter = db->NewIterator(ReadOptions());
			ListsDataKey startdatakey(key, version, currentindex);

			for (iter->Seek(startdatakey.Encode());
				iter->Valid() && currentindex < plistsvalue.GetRightIndex();
				iter->Next(), currentindex++) {
			...
			if (!findpivot) {
				*ret = -1;
				...
				uint64_t midindex =
					plistsvalue.GetLeftIndex() + 
					(plistsvalue.GetRightIndex() - plistsvalue.GetLeftIndex()) / 2;
				if (pivotindex <= midindex) {
					targetindex = (beforeorafter == Before)
						? pivotindex - 1 : pivotindex;

					currentindex = plistsvalue.GetLeftIndex() + 1;
					...
					currentindex = plistsvalue.GetLeftIndex();
					...
					for (afterhalfiter->Seek(startdatakey.Encode());
						afterhalfiter->Valid()
						&& currentindex < plistsvalue.GetRightIndex();
					...
					plistsvalue.ModifyRightIndex(1);
				}
