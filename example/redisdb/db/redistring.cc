...
RedisString::RedisString(RedisDB* redis, const Options& options, const std::string& path)
	:redis(redis),
...
Status RedisString::setvx(const std::string_view &key, 
	const std::string_view &value, const std::string_view &newValue, int32_t *ret, const int32_t ttl) {
	*ret = 0;
	std::string oldValue;
	HashLock hashlock(&lockmgr, key);
	Status s = db->get(ReadOptions(), key, &oldValue);
