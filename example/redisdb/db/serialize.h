...
class StringsMetaValue {
public:
	StringsMetaValue(const std::string_view& value)
		:start(nullptr),
		value(value),
		version(0),
		timestamp(0) {
...
class ParsedStringsMetaValue {
public:
	ParsedStringsMetaValue(std::string* str)
		:value(str),
		version(0),
		timestamp(0) {
	...
	ParsedStringsMetaValue(const std::string_view& str)
		:value(nullptr),
		version(0),
		timestamp(0) {
		if (str.size() >= kStringsValueSuffixLength) {
...
public:
	explicit ListsMetaValue(const std::string_view& uservalue)
		:value(uservalue),
		start(nullptr),
...
public:
	explicit ParsedListsMetaValue(std::string* str)
		:value(str),
		version(0),
		timestamp(0),
		count(0),
	...
	explicit ParsedListsMetaValue(const std::string_view& view)
		:value(nullptr),
		version(0),
		timestamp(0),
		count(0),
...
public:
	ParsedBaseMetaValue(std::string* str)
		...
		version(0),
		timestamp(0) {
		if (str->size() >= kBaseMetaValueSuffixLength) {
			uservalue = std::string_view(str->data(), str->size() - kBaseMetaValueSuffixLength);
			version = DecodeFixed32(str->data() + str->size() - kBaseMetaValueSuffixLength);
	...
	void StripSuffix() {
		if (value != nullptr) {
			value->erase(value->size() - kBaseMetaValueSuffixLength, kBaseMetaValueSuffixLength);
		}
	}

	void SetVersionToValue() {
		if (value != nullptr) {
			char* dst = value->data() + value->size() - kBaseMetaValueSuffixLength;
			EncodeFixed32(dst, version);
		}
	}
