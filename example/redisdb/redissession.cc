...
RedisSession::RedisSession(RedisDb *redis, const TcpConnectionPtr &conn)
        : redis(redis),
          reqtype(0),
          multibulklen(0),
          bulklen(-1),
    ...
    conn->setMessageCallback(std::bind(&RedisSession::proxyReadCallback,
...
void RedisSession::proxyReadCallback(const TcpConnectionPtr &conn, Buffer *buffer) {
...
int32_t RedisSession::processInlineBuffer(const TcpConnectionPtr &conn, Buffer *buffer) {
    const char *newline;
...
int32_t RedisSession::processMultibulkBuffer(const TcpConnectionPtr &conn, Buffer *buffer) {
